...
describe("local staged bundle publication service", () => {
  ...
  it("publishes a new revision and supersedes the previous active revision", () => {
    ...
    const result = publishDraftRevision(publishInput({ revisions: [current, draft] }), driver.dependencies);
  ...
  it("returns the persisted result for an idempotent publication retry", () => {
    const current = revision({ id: firstRevisionId, number: 1, status: "published" });
    const draft = revision({ id: secondRevisionId, number: 2, status: "draft" });
    const driver = driverForCurrentRevision();
    const input = publishInput({ revisions: [current, draft] });
    const first = publishDraftRevision(input, driver.dependencies);
    const writeCount = driver.state.calls.filter((call) => call === "write_snapshot").length;
    const retry = publishDraftRevision(input, driver.dependencies);
  ...
  it("retries a failed publication_id without replacing the active pointer", () => {
    const current = revision({ id: firstRevisionId, number: 1, status: "published" });
    const draft = revision({ id: secondRevisionId, number: 2, status: "draft" });
    const driver = driverForCurrentRevision();
    let failOnce = true;
    ...
    const input = publishInput({ revisions: [current, draft] });
    const failed = publishDraftRevision(input, dependencies);
    const retry = publishDraftRevision(input, dependencies);
  ...
  it("compensates a read-back mismatch by restoring the previous Snapshot", () => {
    const current = revision({ id: firstRevisionId, number: 1, status: "published" });
    const draft = revision({ id: secondRevisionId, number: 2, status: "draft" });
    const driver = driverForCurrentRevision();
    let reads = 0;
    ...
    const result = publishDraftRevision(publishInput({ revisions: [current, draft] }), dependencies);
  ...
  it("compensates an uncertain pointer update failure", () => {
    const current = revision({ id: firstRevisionId, number: 1, status: "published" });
    const draft = revision({ id: secondRevisionId, number: 2, status: "draft" });
    const driver = driverForCurrentRevision();
    const dependencies = {
  ...
  it("compensates pointer and Snapshot after an audit record failure", () => {
    const current = revision({ id: firstRevisionId, number: 1, status: "published" });
    const draft = revision({ id: secondRevisionId, number: 2, status: "draft" });
    const driver = driverForCurrentRevision();
    const result = publishDraftRevision(publishInput({ revisions: [current, draft] }), {
      ...driver.dependencies,
  ...
  it("reports a compensation failure instead of silently claiming recovery", () => {
    const current = revision({ id: firstRevisionId, number: 1, status: "published" });
    const draft = revision({ id: secondRevisionId, number: 2, status: "draft" });
    const driver = driverForCurrentRevision();
    let reads = 0;
  ...
  it("detects external pointer drift before switching the active revision", () => {
    const current = revision({ id: firstRevisionId, number: 1, status: "published" });
    const draft = revision({ id: secondRevisionId, number: 2, status: "draft" });
    const driver = driverForCurrentRevision();
    const result = publishDraftRevision(publishInput({ revisions: [current, draft] }), {
  ...
  it("rejects a parity gate failure before any external write", () => {
    const draft = revision({ id: firstRevisionId, number: 1, status: "draft" });
    const driver = createInMemoryPublicationDriver();
    const result = publishDraftRevision(
      publishInput({ activeRevisionId: null, revisionId: firstRevisionId, revisions: [draft] }),
  ...
  ])("stops %s gate failures before any external write", (failedStep, overrides) => {
    const draft = revision({ id: firstRevisionId, number: 1, status: "draft" });
    const driver = createInMemoryPublicationDriver();
    const result = publishDraftRevision(
      publishInput({ activeRevisionId: null, revisionId: firstRevisionId, revisions: [draft] }),
  ...
  it("rolls back to a superseded revision using its validated historical Snapshot", () => {
    ...
    const result = rollbackPublishedRevision({
