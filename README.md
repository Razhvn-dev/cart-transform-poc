...
POC-02 is complete and verified end to end in the development store.

- Product Builder selecting Fuel Test writes `_builder_fuel_variant_id` as `gid://shopify/ProductVariant/51505348346134`.
- Product Builder selecting Fuel Test 2 writes `_builder_fuel_variant_id` as `gid://shopify/ProductVariant/51518319591702`.
- Cart Transform expands Master Kit into EFI Test, the selected Fuel component, and Coil Test.
- Fuel Test 2 was verified through Storefront, `/cart.js`, Function input/output logs, Checkout, and Order Admin.
- Order Admin shows each expanded component as `Part of: Master Kit Test`.

### POC-02 root causes and resolution

- `extensions/product-builder/blocks/product-builder.liquid` originally defined three `type: "product"` settings, which exceeded Shopify's limit for this app block and prevented the dev preview bundle from updating.
- The Product Builder block now keeps only the `master_product` picker.
- Fuel Test and Fuel Test 2 use fixed complete POC Variant GIDs in the current Theme App Extension implementation.
- The Master Kit variant originally still had static `productVariantComponents` from POC-01: EFI Test, Fuel Test, and Coil Test.
- That static bundle relationship caused Checkout to keep showing Fuel Test even when Cart Transform output selected Fuel Test 2.
- Removing the Master Kit static component relationships made Cart Transform `expandedCartItems` the only component source for POC-02.
