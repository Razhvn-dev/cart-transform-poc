...
function buildGroupIndexes(snapshot) {
  ...
  for (const group of snapshot.groups) {
    optionsByGroup.set(group.key, new Map(group.options.map((option) => [option.key, option])));
  }
...
function selectDefaults(snapshot, groups) {
  ...
  for (const group of snapshot.groups) {
    ...
    const fallbackOption = options.get(group.default_option) || group.options[0];
...
function applyCompatibilityRules(snapshot, groups, selections) {
  for (const rule of snapshot.rules) {
    if (!conditionsMatch(rule, selections) || rule.effect !== "allow") continue;
...
function allocateDiscountedPrices(components, basisPoints) {
  const subtotalCents = components.reduce((total, component) => total + component.priceCents, 0);
  const finalTotalCents = subtotalCents - calculatePercentageDiscountCents(subtotalCents, basisPoints);
  ...
  const allocatedTotalCents = allocatedPrices.reduce((total, priceCents) => total + priceCents, 0);
  const deltaCents = finalTotalCents - allocatedTotalCents;
  if (allocatedPrices.length > 0) allocatedPrices[allocatedPrices.length - 1] += deltaCents;
